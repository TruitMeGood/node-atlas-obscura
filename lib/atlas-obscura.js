...
const { parseItems, parseTags, parseItinerary, getPaginationNextLink } = require('./utils')
...
class AtlasObscura {
	...
	async getItineraries({city, country}) {
		try {
			const place = slugify(`${city} ${country}`)
			let placeUrl = `/things-to-do/${place}`
			const placeBody = await this.getPlacesRequest('https://www.atlasobscura.com/unusual-trips/berlin-sep-2019')
			return await parseItinerary(placeBody)
		} catch (err) {
			console.log(err)
		}
	}
